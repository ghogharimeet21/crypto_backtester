...
from commons.utils import seconds_to_hms, date_to_ms
...
class Trade:
    ...
    def __init__(
        ...
        # Risk management — stop_loss/target are stored as points away from
        # entry; stop_loss_price/target_price are the derived absolute levels.
    ...
    def close(
        ...
        # Spot markets run 24/7, so trades can span multiple days — using
        # exit_time - entry_time alone (intraday-only assumption) would go
        # negative for any trade held past midnight.
        entry_ts = date_to_ms(self.entry_date) // 1000 + self.entry_time
        exit_ts = date_to_ms(exit_date) // 1000 + exit_time
        self.holding_seconds = exit_ts - entry_ts
